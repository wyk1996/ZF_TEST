...
//显示数据数据地址从0x1000开始
#define DIS_ADD(id,add)    					    ( ( ( (id) & 0x00ff) << 7 ) | ( (add) & (0x007f) ) + 0x1000 ) //0x19 & 0x007f = 0x19 0x19+0x1000=0x1019
...
uint8_t Billbuf[GUN_MAX][300] = {0};			//缓冲
#if(WLCARD_STATE)
...
uint8_t Munu13_ShowSysInfo(void)
{
	...
	time = ((DisSysConfigInfo.DevNum_zf & 0x00ff) << 8 )| ((DisSysConfigInfo.DevNum_zf & 0xff00) >> 8);
    ...
    memcpy(&show_time[0],&time,sizeof(time));
	
    PrintStr(DIS_ADD(HYMenu13.FrameID,9),show_buf,sizeof(show_buf));
    PrintStr(TIMER_ADDR,show_time,sizeof(show_time));
 ...
 * @note
 */
static uint8_t Zf_Timer_Set(uint16_t addr,uint8_t *pvalue,uint8_t len)
{
    uint16_t temp; //记录缓存
    if((pvalue == NULL) || (len != 2) )
    {
    ...
    temp = (pvalue[0] << 8) | (pvalue[1]);
    DisSysConfigInfo.DevNum_zf=temp; //这个主要是存储在结构体中
...
const _DISP_ADDR_FRAME Disp_RecvFrameTable[] =
{
    ...
    {TIMER_ADDR,	 Zf_Timer_Set}, //接受超时时间设置
};
...
static void dwin_show_state(void)
{
    ...
    if(show)
    {
		...
		uint8_t visonbuf[17]={0};
		memcpy(visonbuf,DisSysConfigInfo.DevNum,16); //单独显示上海益虫平台的二维码  //程序版本
		visonbuf[16]=0x30;
        PrintStr(0x1050,(uint8_t*)visonbuf,17); //显示桩编号
    ...
    if(NET_YX_SELCT == XY_HY)
    {
		...
		else if(show_HYXY_name == show_YC)     //需要显示上海益虫二维码
        {
            ...
            memcpy(code,DisSysConfigInfo.DevNum,sizeof(DisSysConfigInfo.DevNum));
            //code[14] = '0';
			code[sizeof(DisSysConfigInfo.DevNum)] = '0';
            memcpy(&buf[len],code,sizeof(DisSysConfigInfo.DevNum)+1);  //显示益虫二维码
            if((strncmp(s_qr_buf,(char *)buf ,sizeof(DisSysConfigInfo.DevNum)+1+len)))
            {
                memcpy(s_qr_buf,buf,sizeof(DisSysConfigInfo.DevNum)+1+len);
                DisplayQRCode(0x5000,buf,sizeof(DisSysConfigInfo.DevNum)+1+len);
            }
        }
    }
...
void rt_dwin_state_dispose(uint8_t state)
{
    ...
    if((APP_GetSIM7600Status() == STATE_OK) && (APP_GetModuleConnectState(0) == STATE_OK)) //连接上服务器
    {
        ...
        {
			#if(NET_YX_SELCT == XY_YKC)
			    mq_service_send_to_4gsend(APP_SJDATA_QUERY,GUN_A ,0 ,NULL);  //YKC变位上送
			#elif(NET_YX_SELCT == XY_HY)
			    mq_service_send_to_4gsend(APP_STE_DEVS,0,0,NULL); //HY实时查询变位上送
			#endif
			laststate = state;
        }
    }
...
//            (DisSysConfigInfo.IP[2] != NetConfigInfo[NET_YX_SELCT].IP[2]) && (DisSysConfigInfo.IP[3] != NetConfigInfo[NET_YX_SELCT].IP[3]))
	...
	else if(show_HYXY_name == show_YC)   //益中平台IP固定不变，不能修改，每上电一次就会写入固定IP
	{
//        DisSysConfigInfo.IP[0]= NetConfigInfo[NET_YX_SELCT].IP[0];
//        DisSysConfigInfo.IP[1]= NetConfigInfo[NET_YX_SELCT].IP[1];
//        DisSysConfigInfo.IP[2]= NetConfigInfo[NET_YX_SELCT].IP[2];
...
//		DisSysConfigInfo.standaloneornet = DISP_NET; //YC一上电就是固定网络模式下
